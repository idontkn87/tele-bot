...
async def get_powered_by_text() -> str:
    if not CLONE_MODE:
        return ""
    username = (MASTER_USERNAME or await get_setting("master_username", "")).lstrip("@").strip()
    return f"⚡ Powered by @{username}" if username else "⚡ Powered by Master"
...
async def claim_number_for_user(user_id: int) -> Optional[str]:
    """Pick ONE number for this user, honoring the rules:
      • never the same number twice for the same user,
      • never a number already handed to MAX_USERS_PER_NUMBER users,
      • spread out: choose among the LEAST-used eligible numbers, then random,
        so users don't all get the same one first.
    Returns the canonical number string, or None if the pool is exhausted.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        await db.execute("BEGIN IMMEDIATE")
        cursor = await db.execute(
            """
            SELECT rn.id, rn.number, rn.handout_count
            FROM reward_numbers rn
            WHERE rn.handout_count < ?
              AND rn.id NOT IN (
                    SELECT number_id FROM reward_handouts WHERE user_id = ?
              )
            ORDER BY rn.handout_count ASC
            """,
        ...
        if not rows:
            return None
        # All rows with the minimum handout_count are equally "spread" — pick
        # randomly among them so distribution isn't sequential.
        min_count = rows[0]["handout_count"]
        candidates = [r for r in rows if r["handout_count"] == min_count]
        chosen = random.choice(candidates)

        await db.execute(
            "UPDATE reward_numbers SET handout_count = handout_count + 1 WHERE id = ?",
            (chosen["id"],),
        )
        await db.execute(
            "INSERT OR IGNORE INTO reward_handouts (user_id, number_id, sent_at) "
            "VALUES (?, ?, ?)",
            (user_id, chosen["id"], datetime.now(timezone.utc).isoformat()),
        )
        await db.commit()
...
async def reset_all_referrals() -> int:
    """Zero every user's progress WITHOUT deleting anyone. Returns user count.
    Reward-number handout history is wiped too so numbers can flow again."""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def get_stats() -> dict:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        rewards = await count("SELECT COUNT(*) FROM users WHERE reward_sent=1")
...
async def maybe_credit_referral(user_id: int, bot: Bot) -> None:
    """Credit the referrer once the invited user passes EVERY enabled step.
    Banned/restricted users can neither give nor receive credit."""
    user = await get_user(user_id)
    if (
        user is None
        or user["referred_by"] is None
        or user["referral_credited"]
        or user["banned"]
        or user["restricted"]
    ):
        return

    if not await all_verifications_passed(user):
        return

    referrer = await get_user(user["referred_by"])
    if referrer is None or referrer["banned"]:
        return
    ...
    # Each COMPLETED referral hands out one number, provided the target is met.
    if count >= required:
        await deliver_number_reward(bot, referrer_id, count)
    else:
        try:
            await bot.send_message(
                referrer_id,
                f"🎉 <b>+1 referral!</b>\n\n"
                f"{progress_bar(count, required)}  {count}/{required}\n"
                f"Only {max(0, required - count)} more to unlock your reward 🚀",
            )
        except Exception:
            pass
...
async def protected_feature_for_event(event, data) -> str:
    ...
    exact = {
        ...
        "adm_required":"referral","adm_setadmin":"settings","adm_export":"csv_export",
...
async def admin_panel_keyboard(admin_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 Reward Caption", callback_data="adm_reward"),
         InlineKeyboardButton(text="🔢 Required Refs", callback_data="adm_required")],
...
async def render_flow(
    ...
    elif step == STEP_CAPTCHA:
        await render_captcha(bot, chat_id, user_id, edit_message=edit_message)
    elif step == STEP_PHONE:
        await render_phone(bot, chat_id)
    else:
...
async def cb_captcha(callback: CallbackQuery, bot: Bot) -> None:
    ...
    if user["captcha_answer"] and chosen == user["captcha_answer"]:
        ...
        try:
            await callback.message.edit_text("✅ <b>Verified — you're human!</b>")
        except TelegramBadRequest:
            pass
        await callback.answer("✅ Correct!")
        await maybe_credit_referral(user_id, bot)
        await render_flow(bot, callback.message.chat.id, user_id)
    else:
...
async def cmd_admin(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(await ui_message("admin_panel"), reply_markup=await admin_panel_keyboard())


@admin_router.callback_query(F.data == "adm_back")
async def cb_admin_back(callback: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await callback.message.edit_text(await ui_message("admin_panel"), reply_markup=await admin_panel_keyboard(callback.from_user.id))
    await callback.answer()
...
async def process_ui_button(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(await ui_message("button_saved"), reply_markup=await admin_panel_keyboard())
...
async def cb_user_add_referral(callback: CallbackQuery, bot: Bot) -> None:
    ...
    if user is not None:
        required = await get_required_referrals()
        if user["referral_count"] >= required:
